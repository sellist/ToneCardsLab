...
from tcl_api.models.file import FileRead, UserFilesResponse
...
@router.get("/deck/{deck_id}", response_model=ApiResponse[List[FileRead]])
def get_deck_files(deck_id: UUID, db: Session = Depends(get_db)):
    ...
    return ApiResponseBuilder.ok().data([f.model_dump() if hasattr(f, 'model_dump') else f for f in files_data]).build()
...
@router.post("/bulk-upload", response_model=ApiResponse[List[FileRead]])
